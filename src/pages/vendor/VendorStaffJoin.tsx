...
export default function VendorStaffJoin() {
  ...
  const handleAcceptInvite = async () => {
    ...
    if (user) {
      await linkUserToVendor(user.id);
      return;
    }
    ...
    if (isSignUp) {
      ...
      try {
        const { data, error } = await supabase.auth.signUp({
          ...
          password,
          options: {
            emailRedirectTo: `${window.location.origin}/vendor/staff/join/${code}`,
        ...
        if (data.user) {
          await linkUserToVendor(data.user.id);
        } else {
  ...
  const linkUserToVendor = async (userId: string) => {
    if (!invite) return;

    setJoining(true);
    try {
      // Update the staff record with the user's ID and mark as accepted
      ...
      toast({ title: 'Welcome to the team!', description: `You've joined ${invite.vendor_name}` });
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardTitle className="flex items-center justify-center gap-2">
            ...
            Join {invite?.vendor_name}
          </CardTitle>
          <CardDescription>
            You've been invited to join as a
            <Badge className="ml-2 capitalize">{invite?.role}</Badge>
          </CardDescription>
        </CardHeader>
