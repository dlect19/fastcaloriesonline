...
import { ArrowLeft, Pill, Clock, Check, Bell, AlertTriangle, Loader2 } from 'lucide-react';
...
export default function DrugTracker() {
  ...
  const activeUsage = usageRecords.filter(u => !u.is_completed);
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="p-4 space-y-6">
        ...
        {/* Completed */}
        {completedUsage.length > 0 && (
          <div>
            <h2 className="font-semibold text-foreground mb-3 flex items-center gap-2">
              <Check className="w-5 h-5 text-calorie-low" /> Completed Courses ({completedUsage.length})
            </h2>
            <div className="space-y-2">
              {completedUsage.map(record => (
                <Card key={record.id} className="opacity-70">
                  <CardContent className="p-3 flex items-center justify-between">
                    <div>
                      <p className="font-medium text-sm text-foreground">{record.drug_name}</p>
                      <p className="text-xs text-muted-foreground">{record.total_doses} doses completed</p>
                    </div>
                    <Badge variant="outline" className="text-xs text-calorie-low border-calorie-low">✅ Done</Badge>
                  </CardContent>
                </Card>
              ))}
            </div>
