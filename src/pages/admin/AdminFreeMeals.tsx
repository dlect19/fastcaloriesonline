...
import { Plus, Trash2, Gift, UtensilsCrossed, Loader2, Store, Package } from 'lucide-react';
...
export default function AdminFreeMeals() {
  ...
  // Auto-fill meal value from product price
  useEffect(() => {
    if (selectedProduct) {
  ...
  const handleCreate = async () => {
    ...
    setSaving(true);
    const { error } = await supabase.from('free_meal_promos').insert({
    ...
    if (error) {
      ...
      toast({ title: 'Free meal promo created!' });
      setShowDialog(false);
      resetForm();
      fetchPromos();
    }
