...
BEGIN
  IF NEW.payment_status = 'paid' AND (OLD.payment_status IS NULL OR OLD.payment_status != 'paid') THEN
    ...
    IF v_platform_wallet_id IS NOT NULL AND v_company_revenue > 0 THEN
      ...
      -- FIX: platform wallet id goes into platform_wallet_id (FK to platform_wallet),
      -- NOT wallet_id (FK to wallets). The previous version caused FK violations
      -- that aborted the entire order-paid transaction, leaving orders stuck pending.
      INSERT INTO wallet_transactions (
        wallet_type, category, transaction_type, amount, order_id,
