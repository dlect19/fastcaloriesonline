...
-- Fix both triggers: replace resolve_commission_rate fallback with clamped fee model for platform riders
-- The bug: when credit_vendor_on_payment fires before rider_payout_details exists,
-- it falls back to percentage-based commission instead of the clamped fee model.
...
BEGIN
  IF NEW.payment_status = 'paid' AND (OLD.payment_status IS NULL OR OLD.payment_status != 'paid') THEN
    ...
    IF v_promo_discount > 0 THEN
      IF v_is_test THEN
        UPDATE platform_wallet SET test_balance = COALESCE(test_balance, 0) - v_promo_discount, updated_at = NOW()
        WHERE id = v_platform_wallet_id;
      ELSE
        UPDATE platform_wallet SET balance = COALESCE(balance, 0) - v_promo_discount,
          total_earned = COALESCE(total_earned, 0) - v_promo_discount, updated_at = NOW()
        WHERE id = v_platform_wallet_id;
      END IF;
    ...
    IF NEW.rider_id IS NOT NULL AND v_delivery_fee > 0 THEN
      ...
      IF v_existing_rider_tx IS NULL THEN
        ...
        ELSIF v_affiliated_vendor_id IS NOT NULL THEN
          -- Vendor-affiliated rider: clamped fee
        ...
        ELSE
          -- Platform/independent rider: ALWAYS use clamped fee model (NO percentage fallback)
          SELECT rpd.final_rider_pay, rpd.platform_fee
          INTO v_payout_final_pay, v_payout_platform_fee
          FROM rider_payout_details rpd
          WHERE rpd.order_id = NEW.id
          ...
          IF v_payout_final_pay IS NOT NULL AND v_payout_final_pay > 0 THEN
            ...
            v_rider_commission_rate := CASE WHEN v_delivery_fee > 0 
              ...
              ELSE 0 END;
          ELSE
            -- FIXED: Use clamped fee model instead of resolve_commission_rate percentage
            SELECT COALESCE((SELECT value::NUMERIC FROM platform_settings WHERE key = 'rider_platform_fee_pct'), 20) INTO v_fee_pct;
            SELECT COALESCE((SELECT value::NUMERIC FROM platform_settings WHERE key = 'rider_platform_fee_min'), 300) INTO v_fee_min;
            SELECT COALESCE((SELECT value::NUMERIC FROM platform_settings WHERE key = 'rider_platform_fee_max'), 700) INTO v_fee_max;
...
-- Also fix credit_rider_on_assignment trigger for the same fallback issue
...
BEGIN
  IF NEW.rider_id IS NOT NULL 
    ...
    IF v_delivery_company_id IS NOT NULL THEN
      -- Delivery company rider: use resolve_commission_rate (percentage-based is correct for logistics)
