...
-- 1. Backfill order_financials.outlet_id from orders where missing
UPDATE order_financials of
SET outlet_id = o.outlet_id
FROM orders o
WHERE of.order_id = o.id
  AND of.outlet_id IS NULL
  AND o.outlet_id IS NOT NULL;

-- 2. Update credit_vendor_on_payment trigger to be outlet-aware
...
BEGIN
  IF NEW.payment_status = 'paid' AND (OLD.payment_status IS NULL OR OLD.payment_status != 'paid') THEN
    ...
    -- Find outlet-specific wallet (or fallback to vendor-level wallet)
    IF NEW.outlet_id IS NOT NULL THEN
      ...
      -- Auto-create outlet wallet if missing
    ...
    -- Include outlet_id in order_financials
    ...
    IF NEW.rider_id IS NOT NULL AND v_delivery_fee > 0 THEN
      ...
      IF v_existing_rider_tx IS NULL THEN
        ...
        ELSIF v_affiliated_vendor_id IS NOT NULL THEN
          ...
          -- For vendor-affiliated riders, route delivery revenue to the ORDER's outlet wallet
          IF NEW.outlet_id IS NOT NULL THEN
