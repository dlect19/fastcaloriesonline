...
-- Fix credit_vendor_on_payment trigger: remove min payout guarantee, keep simple clamped fee model
    ...
    IF NEW.rider_id IS NOT NULL AND v_delivery_fee > 0 THEN
      ...
      IF v_existing_rider_tx IS NULL THEN
        ...
        ELSIF v_affiliated_vendor_id IS NOT NULL THEN
          -- Vendor-affiliated rider: simple clamped fee (NO min payout guarantee)
          ...
          IF v_payout_final_pay IS NOT NULL AND v_payout_final_pay > 0 THEN
            -- Use payout details but cap rider_share to delivery_fee
            ...
            v_rider_commission_rate := CASE WHEN v_delivery_fee > 0 
              ...
              ELSE 0 END;
          ELSE
            -- Inline hybrid calculation: clamp(fee * pct%, min, max), NO min payout
          ...
          WHERE order_id = NEW.id;
        ELSE
          -- Platform/independent rider: simple clamped fee (NO min payout guarantee)
          SELECT rpd.final_rider_pay, rpd.platform_fee
          INTO v_payout_final_pay, v_payout_platform_fee
          FROM rider_payout_details rpd
          WHERE rpd.order_id = NEW.id
...
-- Fix credit_rider_on_assignment trigger: remove min payout guarantee
