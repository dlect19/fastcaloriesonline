  ...
  IF NEW.payment_status = 'paid' AND (OLD.payment_status IS NULL OR OLD.payment_status != 'paid') THEN
    ...
    IF NEW.rider_id IS NOT NULL AND v_delivery_fee > 0 THEN
      ...
      IF v_existing_rider_tx IS NULL THEN
        ...
        ELSE
          -- NON-COMPANY RIDERS (including vendor-affiliated): Credit rider's OWN wallet
          ...
          IF v_affiliated_vendor_id IS NOT NULL AND v_affiliated_vendor_id = NEW.vendor_id THEN
            -- Find the vendor's wallet (already resolved above as v_vendor_wallet_id)
            v_affiliated_vendor_wallet_id := v_vendor_wallet_id;
            
            -- Credit vendor_rider_share to vendor wallet
            IF v_is_test THEN
