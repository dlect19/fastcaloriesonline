...
-- Task 1: Update credit_vendor_on_payment to credit vendor-affiliated riders to their OWN rider wallet
-- Task 2: Update credit_rider_on_assignment similarly
-- Both triggers currently route vendor-affiliated rider earnings to vendor's rider_revenue_balance
-- We need to route them to the rider's own wallet instead
...
BEGIN
  ...
  IF NEW.rider_id IS NOT NULL 
    ...
    ELSE
      -- ALL NON-COMPANY RIDERS (including vendor-affiliated): Credit rider's OWN wallet
    ...
    -- Platform delivery commission (always)
    IF v_is_test THEN
...
-- Also update credit_vendor_on_payment to match (the rider section within it)
