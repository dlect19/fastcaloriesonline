-- Create a trigger function that deducts wallet balance when a payout_request is inserted
...
-- Create the trigger on payout_requests
DROP TRIGGER IF EXISTS trigger_deduct_wallet_on_payout ON payout_requests;
CREATE TRIGGER trigger_deduct_wallet_on_payout
  AFTER INSERT ON payout_requests
  FOR EACH ROW
  EXECUTE FUNCTION deduct_wallet_on_payout_request();

-- Also create a trigger to restore balance when payout is rejected/failed
...
DECLARE
  ...
  v_amount NUMERIC;
BEGIN
  -- Only run when status changes TO failed or rejected
  IF (NEW.status IN ('failed', 'rejected')) 
     AND (OLD.status NOT IN ('failed', 'rejected')) THEN
...
DROP TRIGGER IF EXISTS trigger_restore_wallet_on_payout_failure ON payout_requests;
CREATE TRIGGER trigger_restore_wallet_on_payout_failure
  AFTER UPDATE ON payout_requests
  FOR EACH ROW
  EXECUTE FUNCTION restore_wallet_on_payout_failure();