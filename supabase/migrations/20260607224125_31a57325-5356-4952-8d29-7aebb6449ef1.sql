...
-- 1) Replace credit_vendor_on_payment to honor settlement period
...
BEGIN
  IF NEW.payment_status = 'paid' AND (OLD.payment_status IS NULL OR OLD.payment_status != 'paid') THEN
    ...
    -- Credit vendor wallet: hold in pending_balance when settlement is delayed,
    -- otherwise credit balance + eligible immediately.
    IF v_hold THEN
      IF v_is_test THEN
        UPDATE wallets SET
    ...
    INSERT INTO wallet_transactions (
      ...
      wallet_id, environment, status, notes, release_at
    ) VALUES (
      'vendor', 'vendor_share', 'credit', v_vendor_share, NEW.id,
      v_vendor_wallet_id, NEW.environment, v_tx_status,
      CASE WHEN v_hold
           THEN 'Vendor share for order #' || NEW.order_number || ' (held until ' || to_char(v_release_at AT TIME ZONE 'Africa/Lagos', 'YYYY-MM-DD HH24:MI') || ' WAT)'
           ELSE 'Vendor share for order #' || NEW.order_number
      END,
      v_release_at
    );
...
-- 2) Schedule release of pending vendor earnings every 10 minutes
DO $$
BEGIN
  PERFORM cron.unschedule('release-pending-vendor-earnings');
EXCEPTION WHEN OTHERS THEN NULL;
END $$;

SELECT cron.schedule(
  'release-pending-vendor-earnings',
  '*/10 * * * *',
  $$SELECT public.release_pending_vendor_earnings();$$
);
