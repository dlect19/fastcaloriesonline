...
import { imageGenerationWithFallback } from "../_shared/ai-call.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    if (!Deno.env.get("LOVABLE_API_KEY") && !Deno.env.get("GEMINI_API_KEY")) {
      throw new Error("No AI image provider configured (LOVABLE_API_KEY / GEMINI_API_KEY)");
    }
    ...
    const aiResult = await imageGenerationWithFallback(
      "google/gemini-3-pro-image-preview",
      contentParts,
    );

    if (!aiResult.ok || !aiResult.imageDataUrl) {
      console.error("AI image error:", aiResult.status, aiResult.errorText);
      ...
      throw new Error(aiResult.errorText || "No image was generated. Try a different prompt.");
    }

    console.log("[generate-campaign-image] provider=" + aiResult.provider);
    const imageData = aiResult.imageDataUrl;
