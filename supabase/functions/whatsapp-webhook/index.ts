...
serve(async (req) => {
  ...
  try {
    ...
    const runConversationController = async (): Promise<Response | null> => {
      ...
      // Location-aware product discovery. Returns a Response (handled) always.
      const discoverProduct = async (term: string, pendingItems: { name: string; qty?: number | null }[]) => {
        ...
        // Only vendors eligible for THIS customer's location.
